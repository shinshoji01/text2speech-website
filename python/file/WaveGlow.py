...
class WaveGlow(torch.nn.Module):
    ...
    def infer(self, spect, sigma=1.0, audio_noise=None):
        ...
        if audio_noise==None:
            ...
            assert(audio_noise.size(0)==spect.size(0))
            assert(audio_noise.size(1)==self.n_remaining_channels)
            assert(audio_noise.size(2)==spect.size(2))
            assert(audio_noise.device==spect.device)
            assert(audio_noise.dtype==spect.dtype)
            audio = audio_noise
